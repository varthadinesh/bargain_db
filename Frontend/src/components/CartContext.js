...
export const CartProvider = ({ children }) => {
    ...
    useEffect(() => {
        axios.get("http://localhost:8080/addcart")
            .then(response => {
    ...
    const [user, setUser] = useState(() => {
        ...
        const addToCart = (product) => {
            axios.post("http://localhost:8080/addcart", product)
              .then(response => {
    ...
    const addToWishlist = (product) => {
        axios.post("http://localhost:8080/addwishlist", product)
            .then(response => {
    ...
    const removeFromWishlist = (productId) => {
        axios.delete(`http://localhost:8080/wishlist/${productId}`)
            .then(response => {
