...
app.post("/addcart", (req, res) => {
  ...
  const sql = "INSERT INTO cart (`product_id`, `product_type`, `category`, `name`, `image`, `description`, `location`, `color`, `alteration`, `size`, `measurements`, `worn`, `price`, `accepted_by_admin`, `seller_id`, `userid`) values (?)";
  const data = [
    req.body.id,
...
app.post("/saveBillingAddress", (req, res) => {
  ...
  const sql = "INSERT INTO billing_address (firstname, lastname, email, country, state, city, address1, address2, pincode, phone) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
app.post("/saveShippingAddress", (req, res) => {
  ...
  const sql = "INSERT INTO shipping_address (firstname, lastname, email, country, state, city, address1, address2, pincode, phone) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
  const values = [firstname, lastname, email, country, state, city, address1, address2, pincode, phone];
  db.query(sql, values, (err, result) => {
    if (err) {
...
app.post("/updatepayment", (req, res) => {
  ...
  const sql = "UPDATE products SET payment_status = ? WHERE buyer_id = ?";

  db.query(sql, [payment_status, id], (err, data) => {
    if (err) {
      console.log(err);
      return res.json(err);
    }
