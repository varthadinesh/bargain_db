...
app.get("/allproducts", (req, res) => {
  const sql1 = "select * from women";
  const sql2 = "select * from kids";
  const sql3 = "select * from jewellery";
  const sql4 = "select * from books";
  ...
  db.query(sql1, (err, dbdata) => {
    ...
    db.query(sql2, (err, dbdata1) => {
      ...
      db.query(sql4, (err, dbdata3) => {
        if (err) {
          return res.json("Error");
        } else {
          result.data4 = dbdata3;
        }
